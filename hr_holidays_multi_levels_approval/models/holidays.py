...
class Holidays(models.Model):
    ...
    pending_transfered_approver_user = fields.Many2one('res.users', string='Pending transfered approver user',
                                                       compute="_compute_pending_transfered_approver_user", search='_search_pending_transfered_approver_user')
    ...
    def action_validate(self):
        ...
        approbation_obj = self.env['hr.employee.holidays.approbation']
        for holiday in self:
            approbation_obj.create({'holidays': holiday.id, 'approver': self.env.uid, 'date': fields.Datetime.now()})
        super(Holidays, self).action_validate()

    @api.multi
    def _compute_current_user_is_approver(self):
        for holiday in self:
            if holiday.pending_approver.user_id.id == self.env.user.id or holiday.pending_approver.transfer_holidays_approvals_to_user.id == self.env.user.id:
                holiday.current_user_is_approver = True
            else:
                holiday.current_user_is_approver = False
    ...
    def _compute_pending_transfered_approver_user(self):
        for holiday in self:
            holiday.pending_transfered_approver_user = holiday.pending_approver.transfer_holidays_approvals_to_user
